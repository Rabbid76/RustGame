...
pub fn main() {
    ...
    'running: loop {
        ...
        if save_images {
            image.save(canvas.get_surface(), &Path::new("screenshot.png")).unwrap();
            image.save(canvas.get_surface(), &Path::new("screenshot.jpg")).unwrap();
            image.save(canvas.get_surface(), &Path::new("screenshot.bmp")).unwrap();
            image.save(canvas.get_surface(), &Path::new("screenshot.tga")).unwrap();
        }
        /*
        if clock.get_frames() < 10 {
            ...
            image
                .save_frames(&render_frames, Path::new("screenshot.gif"))
                .unwrap();
        }
        */
    }
