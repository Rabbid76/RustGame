...
pub fn surface_to_sdl2_surface<'a>(surface: &'a dyn Surface) -> Result<&'a Sdl2Surface, Box<dyn Error>> {
...
pub unsafe fn sdl2_surface_to_opencv_mat(sdl2_surface: &sdl2::surface::Surface<'static>) -> Result<core::Mat, Box<dyn Error>> {
    let raw_surface = sdl2_surface.raw();
    let w = (*raw_surface).w as i32;
    let h = (*raw_surface).h as i32;
    let step = (w * 4) as usize;
    Ok(core::Mat::new_rows_cols_with_data(h, w, core::CV_8UC4, (*raw_surface).pixels, step)?)
}
